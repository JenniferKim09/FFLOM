'''
python recon_r.py --path ./data_preprocessed/zinc_test_r/ --gen --gen_out_path ./ --batch_size 32 --lr 0.001 --epochs 10 --shuffle --name 0904 --num_flow_layer 12 --nhid 128 --nout 128 --gcn_layer 3 --seed 66666666 --init_checkpoint ./good_ckpt/checkpoint335 --gen_num 100 --edge_unroll 12
'''
...
full_nodes, full_edges, frag_nodes,frag_edges, r_len, v_to_keep, exit_point, full_smi, data_config = read_molecules(args.path)
train_dataloader = DataLoader(CondDataset(full_nodes, full_edges, frag_nodes,frag_edges,r_len, exit_point, v_to_keep, full_smi),
...
def reconstruct(num, x_deq, adj_deq, noise=0):
    ...
    new_nodes=[i for i in range(len(node_symbol),(len(node_symbol)+r_len[num]))] 
    ...
    for i,new_node in enumerate(new_nodes): #[20,21,22,23]
        ...
        new_edges=[i for i,x in enumerate(valences) if x > 0] #skip one exit point
