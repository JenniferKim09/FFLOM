...
'''
args = parser.parse_args(args=['--path' ,'./data_preprocessed/test/',\
    '--gen','--gen_out_path','',\
    '--batch_size','32','--lr','0.001','--epochs','10','--shuffle','--name','',\
    '--num_flow_layer','12','--nhid','128','--nout','128','--gcn_layer','3',\
    '--seed','66666666','--init_checkpoint','./good_ckpt/checkpoint306',
    '--gen_num','100','--edge_unroll','12'])
'''
...
def reconstruct(num, x_deq, adj_deq, noise=0):
    ...
    for i in range(len(node_symbol)):
        if i not in exit_point[num]:
            ...
            valences[i] = 1
    cur_node_features = torch.tensor(frag_nodes[num]).unsqueeze(0).cuda() #(1,38,14)
    cur_adj_features = torch.tensor(frag_edges[num]).unsqueeze(0).cuda() #(1,4,38,38)
    ...
    for i,new_node in enumerate(new_nodes): #[20,21,22,23]
        ...
        for new_edge in new_edges:
            ...
            if new_node <= 24:
                edge = int(new_node*(new_node-1)/2 + new_edge)
            else:
                #edge = int(24*23/2 + (new_node-24+1)*24 - (new_node-new_edge))
                edge = int(300+(new_node-25)*24+(new_edge-new_node+24))
